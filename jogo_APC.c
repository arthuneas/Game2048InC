...
#define arquivo_entrada "numeros.txt"
...
// Funcao para abrir o arquivo de numeros
void abrir_arquivo(const char *nome_arquivo) {

    //abre arquivo para leitura
    arquivo_numeros = fopen(nome_arquivo, "r");
...
// Funcao para ler o proximo numero do arquivo
int ler_numero() {

    int numero;

    if (arquivo_numeros != NULL && fscanf(arquivo_numeros, "%d", &numero) == 1) {
        return numero; // retorna numero lido

    } else {
        return -1; // retorna para indicar EOF
...
void instrucoes() {
    ...
    printf("- -> OBJETIVOS:\n");
    printf("      O objetivo do Jogo de APC e combinar os numeros em um tabuleiro ate o arquivo acabar.\n");
    ...
    printf("      Voce ganha quando o arquivo de numeros chegar ao fim\n\n");
    ...
    printf("      6. O objetivo e ir ate o fim do arquivo.\n\n");
...
void jogar(char *nome) {
    ...
    abrir_arquivo ("numeros.txt"); // Abre o arquivo antes de come�ar o jogo

    // L� os primeiros n�meros do arquivo
    int nro = ler_numero();
    int nro_seguinte = ler_numero();
    ...
    while (leitura != 9) {
        ...
        if (leitura == 9) {
            ...
            fechar_arquivo(); // Fecha o arquivo antes de sair
            return;
        }
        ...
        if (linha_disponivel != -1) {
            ...
            nro = nro_seguinte;
            nro_seguinte = ler_numero();

            // Se o arquivo chegou ao fim, encerra o jogo
            if (nro_seguinte == -1) {
